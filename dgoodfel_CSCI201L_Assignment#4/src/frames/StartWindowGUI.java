...
public class StartWindowGUI extends JFrame {
	...
	public JTextField networkedTeamField;
	...
	public JTextField jnetworkedTeamField;
	...
	// Slider just for the non networked 1-4
	...
	public void createHostCardGUI() {
		...
		AppearanceSettings.setForeground(Color.lightGray, sliderHost);
	...
	private void addListeners() {
		...
		addWindowListener(new ExitStartWindowListener(this));
			...
			public void actionPerformed(ActionEvent e) {
				if (notRadio.isSelected()) {
					...
					if (hostRadio.isSelected()) {
						playersNeeded = sliderHost.getValue();//because one will get deducted with own client
						int port = Integer.parseInt(portField.getText());
						gameData.setNumberOfQuestions(quickPlay.isSelected() ? 5 : 25);
						server = new ChatServer(port, playersNeeded, gameData);
						try{Thread.sleep(600);}
						catch(InterruptedException ie){ie.printStackTrace();}
						client = new ChatClient(StartWindowGUI.this, "localhost", Integer.parseInt(portField.getText()));
						//nwStatusLabel
		...
		exitButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if(joinRadio.isSelected() && (!startGameButton.isEnabled())){
					client.sendMessage(new ChatMessage("join left", "join left"));
				}
				else if(hostRadio.isSelected()){
					client.sendMessage(new ChatMessage("host left", "host left"));
				}
	...
	public void hostLeft(){
		if(joinRadio.isSelected()){
			jportField.setText("port");
			ipField.setText("IP Address");
			jnetworkedTeamField.setText(loggedInUser.getUsername());
			clearDataButton.setEnabled(true);
			startGameButton.setEnabled(true);
			jnwStatusLabel.setText("So sorry, the host cancelled the game! Please choose another game to join");
		}
	}
	...
	private class ExitStartWindowListener extends ExitWindowListener{
		public ExitStartWindowListener(StartWindowGUI main){
			super(main);
		}
