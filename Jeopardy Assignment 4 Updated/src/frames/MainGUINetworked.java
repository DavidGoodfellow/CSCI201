...
public class MainGUINetworked extends MainGUI {
	...
	public MainGUINetworked(ServerGameData gameData, Client client, User loggedInUser) {
		...
		make(gameData);
		t15 = new Timer15(this);
		t15.start();
	}
	
	public void gameBoardHitZero(){
		//call serverGameData to make next turn
		//call correct update to the updatePanel
		//whatever the client call is
		if(currentPanel.equals(mainPanel)){
			System.out.println("Game board hit");
			serverGameData.nextTurn();
			setClock();
			addUpdate("No question was selected. It is now " + serverGameData.getCurrentTeam().getTeamName() +"'s turn.");
			setCurrentTeamLabel(serverGameData.getCurrentTeam().getTeamName());
			reset15();
			if(client.getTeamIndex() != serverGameData.getCurrentTeam().getTeamIndex()){
				disableAllButtons();
			}
			else{
				enableAllButtons();
	...
	//sets the Progress Panel's currentTeamLabel
	//also could be used for top left??
	public void setCurrentTeamLabel(String teamName){
		currentTeamLabel.setText(teamName);
	}
