...
public class FactoryManager implements Runnable , ChangeListener{
	...
	public void loadFactory(Factory inFactory, JTable inTable) {
		...
		stop();
		mFactorySimulation = new FactorySimulation(inFactory, inTable, this);
		mRenderPanel.refresh();
		start();
	}
	
	public void sendOrders(ArrayList<String> orders){
		listener.sendOrders(orders);
	}
	
	public void setClientListener(FactoryClientListener listen){
		listener = listen;
	}
