		...
		try {
			...
			while(true) {
				...
				//going to the coffee shop
				{
					...
					mNextNode = mShortestPath.pop();
					atLocation.await();
					((FactoryCoffeeStation) mDestinationNode.getObject()).orderCoffee(mNumber);
					Thread.sleep(500);
				}
				//going to table
				{
					mDestinationNode = mFactorySimulation.getNode("Dining Table");
					//we know its a dining table
					((FactoryDiningStation) mDestinationNode.getObject()).getSpot();
					mShortestPath = mCurrentNode.findShortestPath(mDestinationNode);
					mNextNode = mShortestPath.pop();
					atLocation.await();
					Thread.sleep(5000);
					((FactoryDiningStation) mDestinationNode.getObject()).returnSpot();
				}
				//build the product
